...
DEFINE FILENAME = &PATH/komplexitaet_fbs.htm
DEFINE MOD_STATISTIC_FBS  = script\module_statistic_fbs.sql
...
DECLARE
  ...
  v_counter				  number(5);
v_gesamt_komp number(6):=TVDMIG.tvd_information.get_complex_application_fbs;
  cursor c_module
    IS
        select m.mod_name,  round(ms.modul_gesammt,1) modul_gesammt
        from     module m
                ,module_stat_fmb_fbs ms
  ...
  cursor c_app_stat is
   ...
   , M.MAX
   , M.SUM
   FROM TVDMIG.MODULE_STATISTIC_SET_VIEW_FBS M;
BEGIN
...
db_report_output.subtitle ('anzahl', 'Applikations Komplexität FBS');
...
if v_gesamt_komp between 35 and 60 then
	...
	db_report_output.text ('<b><font size="7">'||TVDMIG.tvd_information.get_complex_application_fbs||'%</font></b><br>');
  db_report_output.text('</td>');
   
db_report_output.text('<td  width="40%" colspan="2" align="center">');
   db_report_output.text ('<b><font size="4">Top Ten Forms Module Komplexität</font></b><br>');
...
db_report_output.html_table_start;
   ...
   for c in c_module loop
    ...
    db_report_output.html_table (  db_report_output.get_link('mod_stat_fbs_'||lower(c.mod_name)||'.htm', lower(c.mod_name))
                                || v_sep
                                || c.modul_gesammt
                               );
   ...
   db_report_output.html_table_end;
   db_report_output.text (db_report_output.get_link('komplexitaet_fbs_alle.htm', '<b>Übersicht Komplexität aller Module >>></b>'));
...
SET TERMOUT ON
prompt Erstellen des module_statistic_fbs.sql.... 

SET TERMOUT OFF

 
SPOOL &MOD_STATISTIC_FBS
DECLARE
  cursor c_module
    IS
        select m.mod_name
...
BEGIN
   ...
   FOR c IN c_module
   LOOP
    dbms_output.put_line('@@mod_stat_fbs_'||lower(c.mod_name)||'.sql');
    tvd_information.erstelle_modul_stat_fbs_sql(c.mod_name);
   END LOOP;
